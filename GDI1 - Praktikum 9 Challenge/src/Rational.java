...
public class Rational {
	...
	private Rational umrechnung(double ergebnis) {
		...
		return kürzen(new Rational(ergebnisP, ergebnisQ, minus));
	}

	private Rational kürzen(Rational Bruch) {

		double zaehler = Bruch.getErgebnisP();
		double nenner = Bruch.getErgebnisQ();
		int teiler = ggT((int) zaehler, (int) nenner);
	...
	// GRENZWERT
	public Rational grenz(Rational bruch) {

		double ergebnis = 0.0;
		double zaehler = bruch.getP();
		double nenner = bruch.getQ();
		...
		for (int i = 0; ergebnis != (ergebnis + (zaehler / Main.potenzen(nenner, i))); i++) {
			ergebnis += zaehler / Main.potenzen(nenner, i);
